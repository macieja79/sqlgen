...
{
    public partial class MainForm : Form, IUIOutput, IProgress
    {
        ...
        private FormProgress m_progress;

        private bool _showSplash;
        private SplashScreen _splashScreen;
        private OptionsForm m_optionsForm;

        public MainForm(IUIController uiController, IEventAggregator eventAggregator, IMenuFactory menuFactory)
        {
        ...
        void InitMenu(Metaproject.Common.MenuItem menu)
        {
            foreach (var item in menu.Children)
            {
                ...
                if (item.CommandId.IsNotNullNorEmpty())
                {
                    toolstripMenuItem.Tag = item.CommandId;
                    toolstripMenuItem.Click += ComandSelectedHandler;
                }
                
                if (item.Children.IsAny())
                {
                    InitMenu(item, toolstripMenuItem);
                }

                mainMenu.Items.Add(toolstripMenuItem);

            }

        }

        void InitMenu(Metaproject.Common.MenuItem menu, ToolStripMenuItem renderedItem)
        {
            foreach (var item in menu.Children)
            {
                var toolstripMenuItem = new ToolStripMenuItem
                ...
                renderedItem.DropDownItems.Add(toolstripMenuItem);
            }
        }
        ...
        {
            MainForm newWindow = new MainForm(_uiController, _eventAggregator, null);
        ...
        private void ComandSelectedHandler(object sender, EventArgs e)
        {
            ToolStripMenuItem item = sender as ToolStripMenuItem;
            var commandName = (string)item.Tag;
            _uiController.CommandSelected(commandName);
            HideProgress();
        }

        private void M_optionsForm_Closed(object sender, EventArgs e)
        {
            if (null != DialogClosed)
                DialogClosed(DialogStatus.Ok);
        }
        ...
        public void ShowOptionForm(Guid commandGuid, object options)
        {
            try
            {
                if (options is CodeGeneratorToolParameters)
                {
                    var gridForm = new CodeGeneratorGridForm(new ClipboardExcelDataProvider(), _eventAggregator);
                    gridForm.DockPanel = null;
                    var pramForm = new CodeGeneratorForm(_eventAggregator);
                    pramForm.DockPanel = null;

                    gridForm.Show(dockPanel);
                    pramForm.Show(dockPanel);
                    return;
                }

                m_optionsForm = new OptionsForm(commandGuid, _eventAggregator);
                m_optionsForm.AttachParameters(options);
                m_optionsForm.Show(dockPanel);
            }
            catch (Exception e)
            {
                m_outputWindow.AddContent(e.Message);
            }
        }

        public void AddToOutput(string str)
        {
            m_outputWindow.AddContent(str);
        }

        public void AddToOutput(List<string> lines)
        {
            m_outputWindow.AddContent(lines);
        }
        ...
        {
            m_progress.Show();
            m_progress.LabelText = info;
            m_progress.Steps = total;
            m_progress.Step(step + 1);
        }
